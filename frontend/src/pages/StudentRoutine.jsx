...
import { Clock, User, Loader2, Calendar } from 'lucide-react';
...
const StudentRoutine = () => {
  ...
  useEffect(() => {
    const fetchRoutine = async () => {
      try {
        const { data } = await api.get(`/routines/student/${user.id}`);
        setSlots(data);
      } catch (error) {
        console.error("Error loading routine");
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <PageHeader 
        ...
        subtitle={`Weekly schedule for ${user.fullName}`}
        backPath="/dashboard"
      />
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        {slots.length === 0 ? (
            <div className="p-12 text-center flex flex-col items-center justify-center text-slate-500">
                ...
                <h3 className="text-lg font-bold text-slate-700">No Routine Published</h3>
                <p>The class routine for your section hasn't been added yet.</p>
            </div>
        ) : (
                ...
                <tbody className="divide-y divide-slate-200">
                    ...
                    <tr key={day} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        return (
                            ...
                            {activeSlot ? (
                                // 🟢 CLASS CARD
                                <div className="bg-white border-l-4 border-l-indigo-500 shadow-sm border border-slate-200 rounded p-3 h-full flex flex-col justify-between hover:shadow-md transition-shadow group">
                                    <div>
                                        <div className="font-bold text-slate-800 text-sm mb-1 line-clamp-2">
                                            {activeSlot.subjectClass.course.name}
                                        </div>
                                        {/* Teacher Name Display */}
                                        <div className="text-xs text-indigo-600 font-semibold bg-indigo-50 inline-flex items-center gap-1 px-1.5 py-0.5 rounded border border-indigo-100 max-w-full truncate">
                                            <User size={10} />
                                            <span className="truncate">
                                                {activeSlot.subjectClass.teacher ? activeSlot.subjectClass.teacher.fullName : 'No Teacher'}
                                            </span>
                                        </div>
                                    </div>
